...
for (j in 2:J) {
  ...
  #gxc <- prod(dnorm(cholest.u, mu_current,var(sig2))) * dnorm(x[j-1],m,sqrt(v)) Takes too much time..
  #gxp <- prod(dnorm(cholest.u, mu_proposed,var(sig2))) * dnorm(xp,m,sqrt(v))
...
lines(xax,dnorm(xax,mvstar[1],sqrt(mvstar[2])), col = "steel blue")
